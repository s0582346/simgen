...
# Initializing nodes
SRC= Source(env, id="SRC",  inter_arrival_time=0.1,blocking=True, out_edge_selection="FIRST_AVAILABLE" )

#src= Source(env, id="Source-1",  inter_arrival_time=0.2,blocking=True,out_edge_selection=0 )
MACHINE1 = Machine(env, id="MACHINE1",node_setup_time=0,work_capacity=2,blocking=True, processing_delay=3,in_edge_selection="RANDOM",out_edge_selection="ROUND_ROBIN")

#MACHINE2 = Machine(env, id="MACHINE2",node_setup_time=0,work_capacity=1,blocking=True, processing_delay=4,in_edge_selection="RANDOM",out_edge_selection="ROUND_ROBIN")
SINK= Sink(env, id="SINK")

# Initializing edges

BUFFER1 = Buffer(env, id="BUFFER1", capacity=10000, delay=0, mode="FIFO")
BUFFER2 = Buffer(env, id="BUFFER2", capacity=10000, delay=0, mode="FIFO")
...
stats_df.to_csv("machine_parallelism_w_2.csv", index=False)
print(f"buffertime={SINK.buffertime/SINK.stats['num_item_received']}")
print(f"Throuphput:{SINK.stats['num_item_received']/env.now}")