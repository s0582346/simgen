...
COMBINER = Combiner(env, id="Combiner1",node_setup_time=0,target_quantity_of_each_item=[1,2],blocking=True, processing_delay=0.5,out_edge_selection=0)
...
# Adding connections
BUFFER1.connect(SRC1,COMBINER)
BUFFER2.connect(SRC2,COMBINER)
BUFFER3.connect(COMBINER,MACHINE1)
...
print("Machine1",np.array(COMBINER.time_per_work_occupancy)/env.now)
print("per_thread_total_time_in_processing_state:MACHINE1", COMBINER.per_thread_total_time_in_processing_state)
print("per_thread_total_time_in_blocked_state:MACHINE1",COMBINER.per_thread_total_time_in_blocked_state)

print(f"Machine1 {COMBINER.id} state times: {COMBINER.stats}")
...
buffers = [BUFFER1, BUFFER2, BUFFER3, BUFFER4, BUFFER5, BUFFER6, BUFFER7]
...
stats_df.to_csv("machine_model_circular_stats_ref.csv", index=False)