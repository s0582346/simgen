...
MACHINE1 = Machine(env, id="MACHINE1",node_setup_time=0,work_capacity=1,blocking=True, processing_delay=0.9,in_edge_selection="FIRST_AVAILABLE",out_edge_selection="ROUND_ROBIN")
...
MACHINE3 = Machine(env, id="MACHINE3",node_setup_time=0,work_capacity=1,blocking=True, processing_delay=2.54,in_edge_selection="RANDOM",out_edge_selection="FIRST_AVAILABLE")
SINK= Sink(env, id="SINK")

# Initializing edges
BUFFER1 = Buffer(env, id="BUFFER1", capacity=2, delay=1, mode="FIFO")

BUFFER2 = Buffer(env, id="BUFFER2", capacity=3, delay=0, mode="FIFO")
BUFFER3 = Buffer(env, id="BUFFER3", capacity=3, delay=0, mode="FIFO")

BUFFER4 = Buffer(env, id="BUFFER4", capacity=1, delay=0, mode="FIFO")
BUFFER5 = Buffer(env, id="BUFFER5", capacity=1, delay=0, mode="FIFO")
# Adding connections
...
print(f"num items in buffer1 {BUFFER1.get_occupancy()}")
print(f"num items in buffer2 {BUFFER2.get_occupancy()}")
print(f"num items in buffer3 {BUFFER3.get_occupancy()}")
print(f"num items in buffer4 {BUFFER4.get_occupancy()}")
print(f"num items in buffer5 {BUFFER5.get_occupancy()}")


print(f"Sink {SINK.id} received {SINK.stats['num_item_received']} items.")

print(f"Throuphput:{SINK.stats['num_item_received']/time}")
...
print("per_thread_total_time_in_processing_state:MACHINE3", MACHINE3.per_thread_total_time_in_processing_state)
print("per_thread_total_time_in_blocked_state:MACHINE3",MACHINE3.per_thread_total_time_in_blocked_state)
...
stats_df.to_csv("machine_model2_stats_ref.csv", index=False)