...
MACHINE1 = Machine(env, id="MACHINE1",node_setup_time=0,work_capacity=1,blocking=True, processing_delay=10,in_edge_selection=0,out_edge_selection="ROUND_ROBIN")
MACHINE2 = Machine(env, id="MACHINE2",node_setup_time=0,work_capacity=1,blocking=True, processing_delay=5,in_edge_selection=0,out_edge_selection="ROUND_ROBIN")
...
FLEET1.connect(MACHINE1,MACHINE2)
BUFFER2.connect(MACHINE2,SINK)
...
FLEET1.update_final_fleet_avg_content(time)
...
print(f"Time-average number of items in  {FLEET1.id} is {FLEET1.stats['time_averaged_num_of_items_in_fleet']}")
