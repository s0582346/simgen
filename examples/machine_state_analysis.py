...
# Initializing nodes
SRC= Source(env, id="SRC",  inter_arrival_time=0.5,blocking=True, out_edge_selection="FIRST_AVAILABLE" )

#src= Source(env, id="Source-1",  inter_arrival_time=0.2,blocking=True,out_edge_selection=0 )
MACHINE1 = Machine(env, id="MACHINE1",node_setup_time=0,work_capacity=2,blocking=True, processing_delay=1,in_edge_selection="RANDOM",out_edge_selection="FIRST_AVAILABLE")
...
BUFFER2 = Buffer(env, id="BUFFER2", capacity=1, delay=3, mode="FIFO")
...
stats_df.to_csv("machine_state_analysis.csv", index=False)
print(f"buffertime={SINK.buffertime/tot_items if tot_items > 0 else 0}")
print(f"Throuphput:{SINK.stats['num_item_received']/env.now}")