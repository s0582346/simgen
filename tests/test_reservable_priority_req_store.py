#%%writefile unittest_reservable_priority_store.py
# @title Testbench
import sys, os
...
def store(env):
    ...
    return ReservablePriorityReqStore(env,  capacity=5, )
...
def process_put(env, store, item, results):
    ...
    results.append(("put", env.now, item, success))
...
def process_get(env, store, results):
    ...
    assert get_event.triggered, "Reserve get event should be triggered"
    item = store.get(get_event)
    results.append(("get", env.now, item))
...
def test_concurrent_reserve_put_and_get(capacity, num_processes):
    ...
    store = ReservablePriorityReqStore(env, capacity=capacity)

    results = []  # Track process actions

    # Start multiple put and get processes
    for i in range(num_processes):
        env.process(process_put(env, store, f"Item-{i}", results))
